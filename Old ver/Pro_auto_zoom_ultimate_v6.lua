-- ==============================================================================
-- PRO AUTOZOOM (ULTIMATE COMMUNITY EDITION) - v7 Debug & Logic Patch
...
local function debug_log(msg, throttle)
    if cache.debug_mode then
        if throttle then
            local now = os.clock()
            if now - last_debug_time > 1.0 then -- Only log once per second
...
function script_properties()
    ...
    -- Hardware group renamed to Monitor / Display Settings
...
local function get_source_dimensions(source, use_monitor_fallback)
    ...
    if ssettings then
        -- FIXED BUG: Try parsing the raw text property FIRST before asking OBS for the source width.
        -- OBS width will return a cropped dimension if the zoom filter is currently active.
        local string_keys = {
        ...
        -- Only fallback to obs width detection if regex failed
        if width == 0 or height == 0 then
            ...
            -- Absolute last resort for internal dimensions (might grab cropped size)
...
function update_monitor_from_source(settings, write_to_ui)
    ...
    if w > 0 and h > 0 then
        ...
        if write_to_ui and settings then
            if cache.manual_hw_override then
                -- FIXED BUG: Do not overwrite user UI properties if manual override is active.
...
-- Crop mathematics
-- ------------------------------------------------------------------------------
local function calculate_crop(mx, my, bw, bh, src_w, src_h)
    bw = math.max(bw, 1); bh = math.max(bh, 1)
    local c_aspect = bw / bh

    -- FIXED BUG: If manual override is active, ALWAYS enforce cache.mon_w
...
function script_tick(seconds)
    ...
    -- Fallback to default if values are bad
    if active_mon_w == 0 then active_mon_w = 1920 end
    if active_mon_h == 0 then active_mon_h = 1080 end

    local mx = math.max(0, math.min(tonumber(m_pos.x) - active_off_x, active_mon_w))
    local my = math.max(0, math.min(tonumber(m_pos.y) - active_off_y, active_mon_h))
    ...
    -- Only auto-detect if manual override is OFF and we don't have dimensions
    ...
    target_crop.left, target_crop.top, target_crop.right, target_crop.bottom =
        calculate_crop(mx, my, bw, bh, src_w, src_h)
    ...
    debug_log(string.format("CROP -> L:%.1f T:%.1f R:%.1f B:%.1f | MOUSE: X:%d Y:%d", 
              cur_crop.left, cur_crop.top, cur_crop.right, cur_crop.bottom, mx, my), true)
    ...
    local show = (cache.ind_mode == "Always On") or
                 (cache.ind_mode == "Hotkey Triggered" and internal_ind_active)
